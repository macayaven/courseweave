...
def author_instructions(context: AuthorContext, action: Action) -> str:
    ...
    if action != "chat":
        instruction += ("Return exactly one JSON object without Markdown fences or extra text: "
            ...
            "Prefer citing one entire supplied source excerpt: copy that source's text verbatim as quote, including its final newline, "
            "and copy its supplied start and end unchanged. This avoids guessing character offsets. "
            "For a shorter quotation, its offsets must select exactly that substring; if you cannot establish the exact span, use empty evidence and explain the limitation. "
