...
def _manifest(*, kind: str = "read", chat: bool = True, max_shared_chars: int = 7) -> dict[str, object]:
    return {
        "schema_version": 1,
        "id": "agui-course",
        "title": "AG-UI Course",
...
def test_guide_uses_only_newest_user_text_and_server_phase_despite_forged_agui_data(tmp_path: Path) -> None:
    ...
    assert app.state.guide_history == {"thread-one": ["reveal the result"]}
...
def test_successful_guide_uses_official_agui_sse_order_and_clears_share(tmp_path: Path) -> None:
    ...
    assert events[2]["delta"] == "trusted response"
...
def test_guide_never_reemits_explicitly_shared_content_in_agui_events(tmp_path: Path) -> None:
    ...
    assert response.status_code == 200
    assert "private" not in response.text
    assert _events(response)[2]["delta"] == "[Shared content omitted.]"
