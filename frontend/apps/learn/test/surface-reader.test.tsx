...
const videoSurface = {
    ...
    type: 'video' as const,
    purpose: 'primary' as const,
    src: 'https://video.example.test/intro.mp4',
...
describe('SurfaceReader', () => {
        ...
        const course = makeCourse({
            ...
            modules: [{
                    id: 'module-a', title: 'Module', phases: [
                        makePhase({ id: 'read-a', title: 'First', surfaces: [htmlSurface], teacher: { access: { mode: "disabled", requires: [] }, guidance: { style: { type: "builtin", id: "explanatory" }, hint_level: "none" }, sharing: { allow: [] }, proposals: { allow: [] } }, completion: { requirements: [] } }),
                        makePhase({ id: 'read-b', title: 'Second', surfaces: [duplicateSurface], teacher: { access: { mode: "disabled", requires: [] }, guidance: { style: { type: "builtin", id: "explanatory" }, hint_level: "none" }, sharing: { allow: [] }, proposals: { allow: [] } }, completion: { requirements: [] } })
                    ],
                    description: ""
                }]
        });
    ...
    it('renders a pending destination outcome before context catches up and retains it after confirmation', () => {
        const course = makeCourse({ title: 'Course', modules: [{ id: 'module-a', title: 'Module', phases: [
                        makePhase({ id: 'read-a', title: 'A', surfaces: [htmlSurface], teacher: { access: { mode: "disabled", requires: [] }, guidance: { style: { type: "builtin", id: "explanatory" }, hint_level: "none" }, sharing: { allow: [] }, proposals: { allow: [] } }, completion: { requirements: [] } }),
                        makePhase({ id: 'read-b', title: 'B', surfaces: [videoSurface], teacher: { access: { mode: "disabled", requires: [] }, guidance: { style: { type: "builtin", id: "explanatory" }, hint_level: "none" }, sharing: { allow: [] }, proposals: { allow: [] } }, completion: { requirements: [] } })
                    ], description: "" }] });
    ...
    it('rejects unsolicited, mismatched, and duplicate-id wrong-phase outcomes', () => {
        ...
        const course = makeCourse({ title: 'Course', modules: [{ id: 'module-a', title: 'Module', phases: [
                        makePhase({ id: 'read-a', title: 'A', surfaces: [htmlSurface], teacher: { access: { mode: "disabled", requires: [] }, guidance: { style: { type: "builtin", id: "explanatory" }, hint_level: "none" }, sharing: { allow: [] }, proposals: { allow: [] } }, completion: { requirements: [] } }),
                        makePhase({ id: 'read-b', title: 'B', surfaces: [duplicateSurface], teacher: { access: { mode: "disabled", requires: [] }, guidance: { style: { type: "builtin", id: "explanatory" }, hint_level: "none" }, sharing: { allow: [] }, proposals: { allow: [] } }, completion: { requirements: [] } })
                    ], description: "" }] });
    ...
    it('accepts a delayed destination outcome after a newer matching context', () => {
        const course = makeCourse({ title: 'Course', modules: [{ id: 'module-a', title: 'Module', phases: [makePhase({ id: 'read-b', title: 'B', surfaces: [videoSurface], teacher: { access: { mode: "disabled", requires: [] }, guidance: { style: { type: "builtin", id: "explanatory" }, hint_level: "none" }, sharing: { allow: [] }, proposals: { allow: [] } }, completion: { requirements: [] } })], description: "" }] });
        const destination = { moduleId: 'module-a', phaseId: 'read-b', surface: videoSurface };
        const outcome = { type: 'courseweave.reader.opened.v1' as const, sourceId: 'notebook-a', moduleId: 'module-a', phaseId: 'read-b', surfaceId: 'lesson-video', htmlSource: null };
        const pending = createReaderIntent('notebook-a', destination, 4, 'runtime-a');
    ...
    it.each([
        ...
        render(<SurfaceReader surface={{ ...videoSurface, src: url }} serviceOrigin="https://courseweave.test"/>);
        expect(screen.getByRole('status')).toHaveTextContent('This course surface is unavailable.');
        expect(document.querySelector('video')).toBeNull();
    });
});
