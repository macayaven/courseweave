...
export function ShareDialog({
  ...
  const restore = () => {
    restoreFocus?.() ?? previousFocus.current?.focus();
  };
  useEffect(() => () => restore(), []);
  const cancel = () => {
    onCancel();
  ...
  const confirm = async () => {
    ...
    try {
      ...
      if (
        captured.kind !== kind ||
        Array.from(captured.content).length === 0 ||
        Array.from(captured.content).length > maxChars
      )
  ...
  return (
    ...
    >
      <h2 id={titleId}>Share with Course assistant</h2>
      <p id={descriptionId}>
        CourseWeave uses this content for one run only and excludes it from
        server conversation replay and saved proposals. Provider retention
        follows the provider’s policy. Maximum {maxChars} characters.
      </p>
      <label>
        Share kind
        <select
          ...
          value={kind}
          onChange={(event) => {
            setKind(event.target.value as ShareKind);
            setContent("");
            setNotice(null);
          }}
        >
