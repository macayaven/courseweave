...
export function selectReaderRoute(course: CourseManifest, sourceId: string, outcome: ReaderNavigationOutcome, active?: ReaderCoordinate, expectedParentOrigin?: string): ReaderRoute | null {
...
export function acceptReaderOutcome(course: CourseManifest, pending: ReaderIntent | null, outcome: ReaderNavigationOutcome, active: ReaderCoordinate | null, contextVersion: number, runtimeIdentity: string | null = null, expectedParentOrigin?: string): StoredReaderOutcome | null {
  const current = pending === null ? null : reconcileReaderIntent(pending, pending.sourceId, active, contextVersion, runtimeIdentity);
  const route = current === null ? null : selectReaderRoute(course, current.sourceId, outcome, undefined, expectedParentOrigin);
...
export function useReaderRoute(course: CourseManifest, runtime: TrustedRuntimeConfiguration, active: ReaderCoordinate | null, contextVersion: number): ReaderRouteController {
  ...
  useEffect(() => {
    const listener = (event: MessageEvent<unknown>) => {
      ...
      if (event.source !== window.parent || event.origin !== currentRuntime.expectedParentOrigin) return;
      const allowedVideoUrls = course.modules.flatMap((module) => module.phases.flatMap((phase) => phase.surfaces.flatMap((surface) => surface.type === 'video' && typeof surface.url === 'string' ? [surface.url] : [])));
      const next = parseReaderNavigation(event.data, currentRuntime.expectedParentOrigin, allowedVideoUrls);
      ...
      const accepted = next === null ? null : acceptReaderOutcome(course, reconciled, next, activeRef.current, contextVersionRef.current, runtimeIdentityRef.current, currentRuntime.expectedParentOrigin);
      if (accepted !== null) {
        pending.current = null;
        setStored(accepted);
      }
  ...
  return { route: current === null ? readerSurface(active?.surface ?? null) : selectReaderRoute(course, runtime.sourceId, current.outcome, undefined, runtime.expectedParentOrigin), requestNavigation };
}
