...
export function PredictionCard({ moduleId, phase, state, client, onState, onRefresh }: PredictionCardProps) {
  ...
  return <section aria-label="Prediction"><h2>Record your prediction</h2>
    {recorded ? <p>Prediction recorded</p> : <><p>Record a prediction before requesting results.</p><label>Your prediction<textarea value={text} onChange={(event) => setText(event.target.value)} /></label><Button type="button" disabled={text.trim().length === 0 || pending} onClick={() => void submit()}>{pending ? 'Saving prediction' : 'Save prediction'}</Button></>}
