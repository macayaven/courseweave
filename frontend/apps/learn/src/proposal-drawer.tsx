...
export function ProposalDrawer({ proposals, client, onRefresh, onProposal, drafts: controlledDrafts, onDraftsChange, onDraftChange, recovery = false, sharedRunPending = false, onRecovery }: { proposals: Proposal[]; state: LearnerState; client: ProposalClient; onRefresh(): Promise<void>; onProposal(proposal: Proposal): void; drafts?: Record<string, string>; onDraftsChange?(drafts: Record<string, string>): void; onDraftChange?(hasDraft: boolean): void; recovery?: boolean; sharedRunPending?: boolean; onRecovery?(): void }) {
  ...
  async function request(proposalId: string, action: (signal: AbortSignal) => Promise<Proposal>) {
    ...
    try {
      ...
      try { await onRefresh(); onProposal(proposal); setNotice(null); } catch (error: unknown) {
      ...
      if (conflict) {
        ...
        const status = typeof error === 'object' && error !== null ? (error as { status?: number }).status : undefined;
        if (status === 401 || status === 403) onRecovery?.();
        setNotice('Proposal action could not be completed. Review and try again.');
      }
    } finally {
