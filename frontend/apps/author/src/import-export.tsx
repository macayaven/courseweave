...
function bytesFrom(file: File): Promise<ArrayBuffer> {
  return new Promise((resolve, reject) => {
    ...
    reader.onerror = () =>
      reject(reader.error ?? new Error("Could not read import file."));
...
export async function readImportFile(file: File): Promise<unknown> {
  if (file.size > MAX_IMPORT_BYTES)
    throw new Error("Import files must be 1 MiB or smaller.");
  let text: string;
  try {
    text = new TextDecoder("utf-8", { fatal: true }).decode(
      await bytesFrom(file),
    );
  } catch {
    throw new Error("Import file is not valid UTF-8.");
  }
  let value: unknown;
  try {
...
export function ImportExport({
  ...
  // This assignment intentionally happens during render.  Effects are too late:
  // a promise may settle between a parent re-render and passive-effect cleanup.
  // The effect still owns physical cancellation; this ref owns authority.
  ...
  const change = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (!current.signal.aborted && started === epoch.current)
        setNotice(error instanceof Error ? error.message : "Import failed.");
    } finally {
      if (!current.signal.aborted && started === epoch.current) setBusy(false);
    }
  };
  return (
    <section aria-label="Import and export">
      ...
      <button
        type="button"
        disabled={formattedJson === null}
        onClick={() => {
          if (formattedJson !== null) download(formattedJson);
        }}
      >
        Export courseweave.json
      </button>
