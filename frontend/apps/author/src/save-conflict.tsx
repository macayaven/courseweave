...
export function SaveConflict({ manifest, etag, exists: _exists, dirty: _dirty, validate, put, getLatest, onSaved, onReviewed, onCanonical, remote = null, disabled = false, requestGeneration = 0, connectionEpoch = 0, onRemoteSaved }: {
  ...
  useEffect(() => { controller.current?.abort(); }, [connectionEpoch]);
  const save = async () => {
    ...
    try {
      ...
      onCanonical?.(checked.formatted_json); setLocalRaw(checked.formatted_json);
  ...
  const downloadLocal = () => {
    ...
    const href = URL.createObjectURL(new Blob([localRaw], { type: 'application/json;charset=utf-8' }));
    const anchor = document.createElement('a'); anchor.href = href; anchor.download = 'courseweave.json'; anchor.click(); URL.revokeObjectURL(href);
  };
  const clearConflict = () => { setLatest(null); setStatus('idle'); };
  return <section aria-label="Save and recovery"><h2>Save and recovery</h2>
    ...
    {status === 'conflict' && latest !== null ? <section aria-label="Remote conflict" role="alert"><h3>The remote version changed</h3>
      ...
      <h4>Local canonical JSON</h4><pre>{localRaw ?? 'Local canonical JSON is unavailable until structural validation completes.'}</pre>
      <button type="button" disabled={localRaw === null} onClick={downloadLocal}>Export local courseweave.json</button>
      <h4>Remote canonical JSON</h4><pre>{latest.raw}</pre>
      <button type="button" onClick={() => { clearConflict(); onReviewed?.(latest.etag); }}>Keep my draft after review</button>
      <button type="button" onClick={() => { if (window.confirm('Replace your local draft with the latest saved course?')) { clearConflict(); onSaved(latest); } }}>Use latest</button>
    </section> : null}
  </section>;
}
