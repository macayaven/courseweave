...
function ArrayFields({ label, itemLabel, values, onChange }: { label: string; itemLabel: string; values: string[]; onChange(values: string[]): void }) {
  const items = values.length > 0 ? values : [''];
  return <fieldset><legend>{label}</legend>{items.map((value, index) => <span key={index}><label>{itemLabel} {index + 1}<input aria-label={`${itemLabel} ${index + 1}`} value={value} onChange={(event) => { const next = values.length === 0 ? [''] : [...values]; next[index] = text(event); onChange(next); }} /></label>{values.length > 0 ? <button type="button" onClick={() => onChange(values.filter((_, valueIndex) => valueIndex !== index))}>Remove {itemLabel.toLowerCase()} {index + 1}</button> : null}</span>)}<button type="button" onClick={() => onChange([...values, ''])}>Add {itemLabel.toLowerCase()}</button></fieldset>;
}
function CourseInspector({ state, dispatch }: InspectorProps) {
  ...
  return <fieldset><legend>Course</legend>
    ...
    <label>Entry module<select value={course.entryModuleKey ?? ''} onChange={(event) => dispatch({ type: 'entry.select', moduleKey: text(event) || null })}><option value="">None</option>{course.modules.map((module) => <option key={module.clientKey} value={module.clientKey}>{module.title || module.id || 'Unnamed module'}</option>)}</select></label>
    <fieldset><legend>Policies</legend>
      ...
      <ArrayFields label="Workspace write globs" itemLabel="Workspace write glob" values={course.policies.workspace_write_globs} onChange={(workspace_write_globs) => dispatch({ type: 'policy.update', patch: { workspace_write_globs } })} />
    </fieldset>
  </fieldset>;
}
...
function SurfaceFields({ surface, update, replace }: { surface: DraftSurface; update(patch: Record<string, unknown>): void; replace(surface: AuthorSurface): void }) {
  ...
  if (surface.type === 'terminal') return <>{common}<label>Terminal label<input value={surface.label} onChange={(event) => update({ label: text(event) })} /></label><ArrayFields label="Arguments" itemLabel="Argument" values={surface.argv} onChange={(argv) => update({ argv })} /><label>Working directory<input value={surface.cwd} onChange={(event) => update({ cwd: text(event) })} /></label></>;
