...
export class LabCaptureProvider {
  ...
  private reject(
    requestId: string,
    code: "busy" | "forbidden" | "too_large" | "unavailable",
  ): void {
    if (!this.listening) return;
    this.options.childWindow.postMessage(
      { type: "courseweave.share.capture.rejected.v1", requestId, code },
  ...
  private allowed(request: CaptureRequest): boolean {
    ...
    return (
      phase.teacher.access.mode === "available" &&
  ...
  private read(kind: CaptureKind): { label: string; content: string } | null {
    ...
    if (kind === "selection") {
      ...
      if (
        this.options.activeWidget() === panel &&
