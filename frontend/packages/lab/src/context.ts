...
export function collectWorkspaceMetadata(
  ...
  const metadata: WorkspaceMetadata = {
    ...
    active_cell_tags: [],
    active_path:
      surface?.activePath === null || surface === null
        ? null
  ...
  if (active === options.notebook.currentWidget) {
    metadata.active_path =
      normalizedPath((active as TrackedDocument).context?.path) ??
      metadata.active_path;
    ...
    metadata.active_cell_tags = Array.isArray(tags)
      ? [
          ...new Set(
            tags.filter(
              (tag): tag is string => boundedString(tag, 160) !== null,
            ),
          ),
        ]
      : [];
    ...
    metadata.active_path ??= normalizedPath(
      (active as TrackedDocument).context?.path,
    );
  }
  return metadata;
}
...
interface ContextPublisherOptions {
  ...
  relay: {
    postContext(
      context: { source_id: string; sequence: number } & WorkspaceMetadata,
    ): Promise<Response>;
  };
  childWindow: Window;
...
async function resolvedContext(
  ...
  try {
    ...
    if (
      ...
      return null;
    if (
      !slugOrNull(data.module_id) ||
      !slugOrNull(data.phase_id) ||
      !slugOrNull(data.surface_id) ||
      typeof data.reason !== "string" ||
      !RESOLUTION_REASONS.has(data.reason as ResolvedContext["reason"])
    )
      return null;
...
export class ContextPublisher {
  ...
  async retry(metadata?: WorkspaceMetadata): Promise<void> {
  ...
  private async drain(): Promise<void> {
    ...
    while (
      ...
      const code =
        response.status === 409 ? await responseCode(response) : null;
...
export class ContextObserver {
  ...
  retryCurrent(): Promise<void> {
    ...
    return (
      this.options.publisher.retry?.(metadata) ??
