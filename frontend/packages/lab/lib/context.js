...
function normalizedPath(value) {
    if (typeof value !== 'string' || value.length === 0 || value.length > 1024 || value.trim() !== value || value.startsWith('/') || value.includes('\\') || value.includes('%') || /^[A-Za-z][A-Za-z0-9+.-]*:/.test(value))
        return null;
...
function collectWorkspaceMetadata(options) {
    ...
    if (active === options.notebook.currentWidget) {
        metadata.active_path = normalizedPath(active.context?.path) ?? metadata.active_path;
...
async function resolvedContext(response) {
    try {
        ...
        if (typeof value !== 'object' || value === null || Array.isArray(value))
            return null;
        const data = value;
        if (Object.keys(data).sort().join(',') !== 'module_id,phase_id,reason,surface_id')
            return null;
        if (!slugOrNull(data.module_id) || !slugOrNull(data.phase_id) || !slugOrNull(data.surface_id) || typeof data.reason !== 'string' || !RESOLUTION_REASONS.has(data.reason))
            return null;
        return data;
    }
    catch {
...
class ContextPublisher {
    ...
    acceptedCoordinate() {
        ...
        if (accepted === null || this.desired === null || metadataKey(this.desired) !== accepted.key || accepted.resolved.module_id === null || accepted.resolved.phase_id === null)
            return null;
    ...
    ensureDrain() {
        if (this.flight !== null)
            return this.flight;
        if (this.blocked || this.desired === null || metadataKey(this.desired) === this.accepted?.key)
...
class ContextObserver {
    ...
    start() {
        if (this.started)
            return;
        this.started = true;
        for (const signal of [this.options.shell.currentChanged, this.options.notebook.currentChanged, this.options.editor.currentChanged, this.options.terminal.currentChanged])
