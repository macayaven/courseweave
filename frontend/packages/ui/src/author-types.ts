...
export type AuthorSurface =
  ...
  | { id: string; type: 'notebook'; role: AuthorSurfaceRole; path: string; match?: { cell_ids?: string[]; cell_tags?: string[] } }
...
export interface AuthorManifest { schema_version: 1; id: string; title: string; description: string; entry_module_id: string | null; policies: { content_sharing: 'explicit_only'; durable_mutation: 'proposal_or_direct_student_action'; terminal_execution: 'student_only'; conversation_memory: 'session_only'; max_shared_chars: number; workspace_write_globs: string[] }; modules: AuthorModule[]; }
