...
export async function verifyAuthorRoles({ page, author, research, report, evidence, secrets, brave, correctLesson, project, onDiscovery }) {
  ...
  const cases = [
    ...
    ['source_researcher', 'Use the permitted references to distinguish JSON parsing from application field validation. Name each source and explain what the fetched Python documentation supports and what requires the instructor reference or further evidence. For this source-synthesis request, put the explanation in message and return findings:[] and change:null; a claim-by-claim factual report is a separate task.'],
